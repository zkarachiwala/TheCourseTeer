  ...
  return (
    <article onClick={onClick} style={{ background: 'var(--card-bg)', borderRadius: 'var(--radius-card)', border: `1.5px solid ${selected ? 'var(--accent)' : 'var(--border)'}`, padding: '20px', cursor: 'pointer', display: 'flex', flexDirection: 'column', gap: '10px', boxShadow: selected ? 'var(--shadow-lg)' : 'var(--shadow)', transition: 'all 0.2s' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
        ...
        {course.atarGuaranteed != null && <span style={{ fontSize: '13px', fontWeight: 700, color: atarColor(course.atarGuaranteed), flexShrink: 0 }}>ATAR {course.atarGuaranteed}</span>}
      </div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {onShortlist && (
          <button onClick={e => { e.stopPropagation(); onShortlist() }} style={{ padding: '5px 10px', borderRadius: 'var(--radius-btn)', border: '1.5px solid var(--border)', background: shortlisted ? 'var(--accent)' : 'transparent', color: shortlisted ? 'var(--accent-fg)' : 'var(--text2)', cursor: 'pointer', fontSize: '11px', fontWeight: 600 }}>
